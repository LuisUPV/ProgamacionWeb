 ...
 */
$title = $values = $units = $icon_type = $bgcolor = $css = $custombgcolor = $customtxtcolor = $options = $el_class = '';
...
foreach ( $values as $data ) {
	$new_line = $data;
	$new_line['icon_type'] = isset( $data['icon_type'] ) ? $data['icon_type'] : $defaultIconClass;

	// Enqueue needed font for icon element
	vc_icon_element_fonts_enqueue( $new_line['icon_type'] );

	switch ( $new_line['icon_type'] ) {
		case 'fontawesome':
			$new_line['icon_type'] = $data['icon_fontawesome'];
			break;
		case 'openiconic':
			$new_line['icon_type'] = $data['icon_openiconic'];
			break;
		case 'typicons':
			$new_line['icon_type'] = $data['icon_typicons'];
			break;
		case 'entypo':
			$new_line['icon_type'] = $data['icon_entypo'];
			break;
		case 'linecons':
			$new_line['icon_type'] = $data['icon_linecons'];
			break;
	}
...
foreach ( $graph_lines_data as $line ) {
	...
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
		...
		. '">';
	$output .= '<small class="vc_label"' . $line['txtcolor'] . '><span class="vc_label_icon"><i class="' . esc_attr( $line['icon_type'] ) . '"></i></span>' . $line['label'] . $unit . '</small>';
	if ( $max_value > 100.00 ) {
