...
if(!class_exists("DFWPThemeActivationBase")) {
	class DFWPThemeActivationBase {
		public $key = "C0C02B2A9A1C6DCD";
		private $product_id = "3";
		private $product_base = "rocket";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_rocket_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=rocket_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		private function getParam( $purchase_key, $app_version, $admin_email = '' ) {
			$req               = new stdClass();
			$req->license_key  = 'B5E0B5F8-DD8689E6-ACA49DD6-E6E1A930';
			$req->email        = 'full@gplready.com';
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
			...
			$response = 200;
							...
							$licenseObj = unserialize( $serialObj );
								...
								$responseObj->expire_date   = '01.01.2030';
								$responseObj->support_end   = '01.01.2030';
								$responseObj->license_title = 'Activated';
								$responseObj->license_key   = 'B5E0B5F8-DD8689E6-ACA49DD6-E6E1A930';
