...
class RestAPI
{
    ...
    function restAPIInit(){
        ...
        register_rest_route( 'wpdm/v2', '/alldownloads', array(
            'methods' => 'GET',
            'callback' => array($this, 'dataTable'),
        ) );

        //wpdm/v2/search-package
        register_rest_route( 'wpdm/v2', '/search-package', array(
            'methods' => 'GET',
            'callback' => array($this, 'searchPackages'),
        ) );

        //wpdm/v2/link-templates
        register_rest_route( 'wpdm/v2', '/link-templates', array(
            'methods' => 'GET',
            'callback' => array($this, 'linkTemplates'),
        ) );

        //wpdm/v2/post-templates
        register_rest_route( 'wpdm/v2', '/post-templates', array(
            'methods' => 'GET',
            'callback' => array($this, 'postTemplates'),
        ) );

        //wpdm/v2/categories
        register_rest_route( 'wpdm/v2', '/wpdmcategories', array(
            'methods' => 'GET',
            'callback' => array($this, 'categories'),
        ) );

        //wpdm/v2/layouts
        register_rest_route( 'wpdm/v2', '/layouts', array(
            'methods' => 'GET',
            'callback' => array($this, 'layouts'),
        ) );

        //wpdm/v2/getlayout
        register_rest_route( 'wpdm/v2', '/getlayout', array(
            'methods' => 'GET',
    ...
    function dataTable()
    {
        ...
        foreach ($all_downloads as $download) {
            $package = [];
            $download_link =  Package::userCanDownload($download->ID) ? Package::downloadLink($download->ID, 0, ['template_type'  =>  'link']) : '<a href="'.get_permalink($download->ID).'" class="btn btn-block btn-danger">Unlock</a>';
                ...
                foreach ($data_field_parts  as $part_index => $data_field) {
                    ...
                    switch ($data_field) {
                        case 'thumb':
                            ...
                            break;
                        case 'icon':
                            $package[$data_field] = "<a href='".get_permalink($download->ID)."'>" . Package::icon($download->ID,  true, 'datatable-icon') . "</a>";
                            break;
                        case 'title':
                        ...
                        case (preg_match('/excerpt_.+/', $data_field) ? true : false) :
                            ...
                            break;
                        case 'file_count':
                            $file_count  = Package::fileCount($download->ID);
                            if ($part_index > 0)
                        ...
                        case 'publish_date':
                            ...
                            break;
                        case 'download_link':
                            $package[$data_field] = $download_link ? $download_link : '<button type="button" disabled="disabled" class="btn btn-danger">' . __("Download", "download-manager") . '</button>';
                            break;
                        case 'details_link':
                            $package[$data_field] = '<a href="'.get_permalink($download->ID).'" class="btn btn-secondary">' . __("Download", "download-manager") . '</a>';
                            break;
                        case 'audio_player':
        ...
        wp_send_json(['packages' => $packages, 'pages' => $pages, '_scparams' => Crypt::encrypt($scparams), 'total' => $total]);
        die();
    }
